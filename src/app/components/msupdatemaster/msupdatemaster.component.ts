...
@Component({
  selector: 'app-msupdatemaster',
  templateUrl: './msupdatemaster.component.html',
  styleUrls: ['./msupdatemaster.component.css']
})
export class MsupdatemasterComponent {
  editForm!: FormGroup;
  ...
  ngOnInit(): void {
    ...
    this.editForm = this.fb.group({
      ...
      updatedDate: [this.record?.updatedDate ? this.record.updatedDate.split('T')[0] : '']
    });
  ...
  updateSubCategories(category: string) {
    const found = this.materialCategoriesData.find((item) => item.category === category);
